class SearchController < ApplicationController
    def index
        ...
        @word_typed_only = params[:word_typed_only]
        @results = search_words(query, @min_word_length, @max_word_length, @starts_with, @ends_with, @dont_include, @word_typed_only)
      end
    def search
        @query = params[:query]
        @min_word_length = params[:min_word_length]
      ...
      def search_words(search, min_word_length, max_word_length, starts_with, ends_with, dont_include, word_typed_only)
        File.open("movies.txt", "r+") do |text|
          data = text.readlines.map { |line| line.downcase.chomp.split(/\s+/) }
          @results = []

          if search && search.split.all? { |term| data.flatten.include?(term) }
